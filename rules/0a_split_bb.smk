...
rule splitBBList:
    ...
    run:
        ...
        if "adapterInsert" or "decoderseq" in tmp_recipe:
            #load bb
            ...
            bb_3.to_csv(output.BB_3, sep="\t", header=False, index=False) # Bead Barcode #2
        else:
            shell(
                # touch {output.BB}
                f"""
...
rule insert_adapter_BB_list:
    ...
    run:
        ...
        if "seeker" in recipes:
            ...
            bb_adapter_r1.to_csv(output.BB_ADAPTER_R1, sep="\t", header=False, index=False)

            bb_adapter_map.to_csv(output.BB_ADAPTER_MAP, sep="\t", header=False, index=False)
            bb_adapter_r1_map.to_csv(output.BB_ADAPTER_R1_MAP, sep="\t", header=False, index=False)

        else:
            shell(f"touch {' '.join(output)}")

        shell(
            f"cp {input.BB_MAP} {output.BB_MAP}"
        )
...
def check_vsearch():
    ...
    if stderr.find("vsearch: command not found") > -1:
        print_error("Could not load find VSEARCH -- check installation")
        sys.exit(1)
...
def count_reads(filename, buffer_size=1024 * 1024):
    ...
    with open_func(filename, "rb") as f:  # Open in binary mode
        buf = io.BufferedReader(f, buffer_size=buffer_size)
        for line in buf:
            count += 1
    return count // 4
...
def main(args):
    ...
    print(
        f"adapter_scan version:             adapter_scan_vsearch_v3\n"
    ...
    if common_adapters:
        ...
        print_log(f"Using single_adapter_mode")

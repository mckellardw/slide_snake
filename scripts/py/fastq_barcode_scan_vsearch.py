...
def parse_args():
    ...
    parser.add_argument(
        "--adapters",
        help="Space-delimited list of adapter sequences.",
        type=str,
    )
...
def write_adapters_fasta(adapter_seqs, fasta_out):
    adapters = []
    for i, adapter in enumerate(adapter_seqs):
        entry = SeqRecord(Seq(adapter), id=f"adapter_{i}", name="", description="")

        adapters.append(entry)
    SeqIO.write(adapters, fasta_out, "fasta")
...
def call_vsearch(tmp_fastq, args):
    ...
    vsearch_cmd = f"vsearch --usearch_global {tmp_fasta} --db {args.adapters_fasta} \
    --threads 1 --minseqlength 20 --maxaccepts 5 --id {args.min_adapter_id} --strand plus \
...
def process_batch(fq_in, args):
    tmp_fastq = tup[0]
    args = tup[1]

    tmp_vsearch = call_vsearch(fq_in, args)
...
def write_tmp_fastx_files_for_processing(n_batches, args):
    ...
    write_adapters_fasta(adapter_seqs=args.adapters, fasta_out=args.adapter_fasta)

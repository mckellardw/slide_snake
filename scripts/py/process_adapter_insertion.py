...
def split_and_insert_adapter(
    bc_map, adapter, bc_primer, bc_lengths, recipes, verbosity=2
):
    ...
    if recipe_type == "decoder":
        ...
        log_message(
            f"Adapter insertion for {recipe_type} not yet implemented",
            level="info",
            verbosity=verbosity,
        )
        return None, None
...
def process_adapters(
    ...
    if not should_process_adapters(recipes, recipes_to_split):
        ...
        log_message(
            "No adapter insertion needed for this recipe",
            level="info",
            verbosity=verbosity,
        )
        return

    bc_adapter, bc_adapter_r1 = split_and_insert_adapter(
        bc_map, adapter, bc_primer, bc_lengths, recipes, verbosity
    )
    ...
    if bc_adapter is None or bc_adapter_r1 is None:
        ...
        for output_file in output_files.values():
            Path(output_file).touch()
        log_message(
            "Adapter insertion failed, created empty files",
            level="info",
            verbosity=verbosity,
        )
        return
    ...
    log_message(
        f"Successfully processed adapter insertion for {get_recipe_type(recipes)}",
        level="info",
        verbosity=verbosity,
    )
...
Examples:
  ...
  python process_adapter_insertion.py \\
    ...
    parser.add_argument(
        "--adapter", required=True, help="Internal adapter sequence to insert"
    )

    parser.add_argument(
        "--bc-primer", required=True, help="Forward primer sequence for R1 processing"
    )
...
def main():
    ...
    try:
        ...
        if args.verbosity >= 2:
            print_log(
                f"Loaded barcode map with {len(bc_map)} entries for adapter insertion"
            )
        ...
        if args.verbosity >= 2:
            print_log(
                f"Successfully processed adapter insertion for recipes: {', '.join(recipes) if recipes else 'default'}"
            )

...
def add_gene_tag_to_bam(in_bam, tsv_file, out_bam, tag='GN'):
    ...
    with pysam.AlignmentFile(in_bam, 'rb') as bam_in, pysam.AlignmentFile(out_bam, 'wb', template=bam_in) as bam_out:
        for read in bam_in:
...
def main():
    ...
    parser.add_argument(
        '--tag',
        default='GN', 
        help='Tag to use for gene assignment. Default is "GN".'
    )

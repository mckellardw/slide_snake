...
parser <-
  ArgumentParser(description = "Generate QC plots for single-cell RNA-seq data")
...
if (!(output_extension %in% supported_devices)) {
  stop(paste(
    ...
    output_extension,
    ". Supported types are:",
    paste(supported_devices, collapse = ", ")
  ))
}
...
knee_plot <- ggplot(data.frame(Index = seq_along(sorted_counts),
